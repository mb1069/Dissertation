...
\MEng
\wordcount{13149, as measured using \lstinline{texcount <filename.tex>}}


\abstract{

A system's ability to locate itself in a known physical environment is key to its capacity to interact with the environment in a precise and controlled fashion. The indoor localisation problem has been approached in a variety of ways, ranging from the identification of pre-defined features or topologies to the more general cloud-point matching. \newline

Cloud point matching can be achieved using a variety of algorithms, each with their distinctive benefits and capabilities. This project aims to investigate our ability to optimise evolutionary algorithms for cloud point matching in the context of indoor localisation. We also evaluate the practicality of these algorithms relating to the complexity of the test case's topology and other existing algorithms. We conclude that significant redundancies can be found in some purely genetic heuristics, and suggest further development into the application of landscape analysis to allow the use of algorithms appropriate to the problem's complexity.
}
...
\begin{figure}
	...
	\begin{tikzpicture}
	    \begin{axis}[
	        ...
	        width  = \textwidth,
	        height = 7cm,
	        ylabel = {StDev. of Combined error},
	        xlabel = {Real execution time (s)},
	        ymin=0.001,
	        % ymode=log,
	        grid=both,
...
% From Elite/ELite_Rand_vs_grid.xlsx
\begin{figure}
	\begin{tikzpicture}
		\begin{axis}
		[height=4cm, width=\textwidth, ytick={1,2, 3}, yticklabels={Grid, Elite, Benchmark}, xlabel={Combined error $\times$ execution time}, xmode=log]
...
\begin{figure}
	\begin{tikzpicture}
		\begin{axis}
		[height=4cm, width=\textwidth, ytick={1,2, 3}, yticklabels={Grid-GA, Grid-ICP}, xlabel={Combined error $\times$ execution time}, xmode=log]
...
\begin{figure}
		\begin{tikzpicture}
			\begin{axis}[
				grid=major,
				ymin=0,
				xlabel=Combined error $\times$ execution time,
				ylabel=Frequency,
				width=\textwidth,
				height=6cm,
				xmin=0,
				...
				{data/icp/grid_vs_icp.csv};
				\legend{Grid-ICP, Grid-GA}
			\end{axis}
		\end{tikzpicture}
\caption{Histogram comparing the efficiency of Grid-ICP and Grid-GA algorithms}
\label{fig:grid_vs_icp_hist}
\end{figure}

A paired T-Test across the ICP's pose error $\times$ execution time results and the equivalent results from the range of executions of the grid-initialised algorithms found the means to be statistically different (N=360, p<0.01). This indicates that the GA based algorithm is indeed more efficient, with a mean efficiency of 2.247 against the Grid-ICP's mean of 4.350. \newline
