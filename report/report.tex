...
\begin{figure}
	\begin{tikzpicture}
		...
		\addplot+[
			boxplot prepared={
				median = 49.941, 
				upper quartile = 216.524,
				lower quartile =  6.084,
				upper whisker =  1514.689,
				lower whisker = 0.000108
			},
		] coordinates {};
		\end{axis}
	\end{tikzpicture}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\addplot+[
			boxplot prepared={
				median =0.732880246,
				upper quartile = 2.244455441,
				lower quartile = 0.195485406,
				upper whisker = 672.076147,
				lower whisker = 0.00238918,	
			},
		] coordinates {};	
		...
		\addplot+[
			boxplot prepared={
				median = 49.94087437,
				upper quartile = 216.5239058,
				lower quartile = 6.084449498,
				upper whisker = 1514.687754,
				lower whisker = 0.000108415,
			},
		] coordinates {};
		\end{axis}
	\end{tikzpicture}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\addplot+[
			boxplot prepared={
				median = 1.32912536,
				upper quartile = 5.35733239,
				lower quartile = 0.113970,
				upper whisker = 68.648691,
				lower whisker = 0.00098869,
			},
		] coordinates {};
		\end{axis}
	\end{tikzpicture}
...
The fitness function may be a source of some inaccuracy. \autoref{fig:landscape_peaks} illustrate a high-granularity visualisation of the fitness functions observed in \autoref{fig:landscape} for scan110, demonstrating that the peak of the fitness function does not match the expected pose (-2.184327,2.641909)

\begin{figure}
\begin{subfigure}[b]{0.3\textwidth}
	\resizebox{\linewidth}{!}{
		\includegraphics[width=9cm,keepaspectratio]{images/landscape_precise_0.1.png}
	}
\end{subfigure}
\begin{subfigure}[b]{0.3\textwidth}
	\resizebox{\linewidth}{!}{
		\includegraphics[width=9cm,keepaspectratio]{images/landscape_precise_0.2.png}
	}
\end{subfigure}
\begin{subfigure}[b]{0.3\textwidth}
	\resizebox{\linewidth}{!}{
		\includegraphics[width=9cm,keepaspectratio]{images/landscape_precise_0.5.png}
	}
\end{subfigure}
	\caption[Fitness landscape peaks.]{High-precision sweep over peaks of scan110's fitness landscape, using different tolerances of subsampling (left: 0.1, centre: 0.2, right: 0.5)}
	\label{fig:landscape_peaks}
\end{figure}

The complexity of the fitness function relative to the point density presents some difficulty in creating a feasible implementation; as $O(n^2)$ is not a significant issue when performing an operation few times per run, it becomes a major performance issue when the operation is repated for every individual, at every generation. This issue led to the use of sub-sampled maps, which marginally reduce


% Fitness function does not match peak
	% Possibly due to tolerance? graph at 0.2, 0.1, 0.0

% Inability to explore large maps without equally large population
	% Larger memory usage vs a GA with crossover?


\section{Comparison to ICP}
% If we are requiring a hill climbing operation from n points in a map, why not just run ICP from them?
	% Inherent parallelism, can be ranked using same fitness function
	% Basic ICP can outperform Grid GA 
	% Show stats

% Non-smooth or large environments
	% If environment is single-peaked smooth (show case) then hill climber would be best
	% If large environment, GA's without crossover are no more efficient than multiple attempts at hill climbing
	% However it is possible that an initial GA could explore the map with lower memory usage and higher computational time (potentially beneficial on low-resource chips? )
