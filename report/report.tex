...
% TODO  add table with median and quartiles
...
\pgfplotstableread[col sep=comma]{data/conclusion/refmap.csv}\datatableconclusionmap
\pgfplotstableread[col sep=comma]{data/conclusion/scan.csv}\datatableconclusionscan
...
\begin{figure*}
        \begin{subfigure}[b]{0.475\textwidth}
			\begin{tikzpicture}
			    ...
			    \addplot+table [scatter, only marks, col sep=comma, x=x, y=y] {\datatableconclusionmap};
			    \addlegendentry{Map}

			    \addplot [scatter, only marks, mark=+, mark size=5pt] coordinates {(4.5,1.5)};
			    \addlegendentry{Robot pose}
			    \end{axis}
			\end{tikzpicture}
			\caption{Reference map}
			\label{fig:conclusion_map}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.475\textwidth}  
			\begin{tikzpicture}
			    ...
			    \addplot+ table [scatter, only marks, col sep=comma, x=x, y=y] {\datatableconclusionscan};
			    \addlegendentry{Scan points}
			    \end{axis}
			\end{tikzpicture}
			\caption{Scan points}
			\label{fig:conclusion_scan}   
        \end{subfigure}
		\caption{Example of a local minimas in repeated environments}
		\label{fig:conclusion_experiment}  
\end{figure*}


\pgfplotstableread[col sep=comma]{data/conclusion/graph.csv}\datatableconclusion
...
\begin{figure}
	...
	\begin{tikzpicture}
	    \begin{axis}[
	        ...
	        height = 5cm,
	        ylabel = {Fitness},
	        xlabel = {Translation along X axis},
	        ymin=0,
	        xmin=-1,
	        xmax=4
	    ]
	    \addplot+table [col sep=comma, x=xpos, y=error] {\datatableconclusion};
	    \end{axis}
	\end{tikzpicture}
	\caption[Pose fitness over local minimas]{Pose fitness when translated along X axis}
	\label{fig:conclusion_graph}
\end{figure}
