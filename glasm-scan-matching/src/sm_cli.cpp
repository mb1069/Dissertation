...
void read_simulation_parameters() {
    ...
    alg_info a[MAXALGS];
    std::cout << "Starting parameter read" << std::endl;
    if (verbose_level > 0) {
        std::cout << "[read_simulation_parameters] reading simulation parameters based on file: "
                  << general_parameters_file << std::endl;
    }
    try {
        ...
        my_options.add_options()
                ...
                ("general.ref_scan", po::value<std::string>(&ref_scan_filename)->default_value("scan3"),
                 "reference scan filename")
                ("general.new_scan", po::value<std::string>(&new_scan_filename)->default_value("scan4"),
                 "new scan filename")
                ("general.scan_path", po::value<std::string>(&scan_path)->default_value("./scans/"),
                 "path to folder containing scans")
                ("general.scan_basename", po::value<std::string>(&scan_basename)->default_value("scan"),
                 "base name for scan file/s. For sm_cli format basename is the base name"
                ...
                ("general.scan_format", po::value<std::string>(&scan_format)->default_value("sm_cli"),
                 "format used to save the scans in file.")
                ("general.new_scan_offset", po::value(&new_scan_offset)->default_value(1),
                 "new scan is read from file new_scan_offset positions ahead of ref scan")
                ...
                ("general.skip_first_runs", po::value(&skip_first_runs)->default_value(1),
                 "number of initial repetitions of the same test to skip")
                ("general.draw_scans", po::value<bool>(&draw_scans)->default_value(true),
                 "set true to enable drawing of scans")
                ...
                ("general.ref_scan_color_B", po::value(&ref_scan_color_B)->default_value(30000),
                 "color to draw ref_scan")
                ...
                ("general.new_scan_color_B", po::value(&new_scan_color_B)->default_value(60000),
                 "color to draw ref_scan")
                ("general.res_scan_color_R", po::value(&res_scan_color_R)->default_value(0), "color to draw ref_scan")
                ("general.res_scan_color_G", po::value(&res_scan_color_G)->default_value(60000),
                 "color to draw ref_scan")
                ("general.res_scan_color_B", po::value(&res_scan_color_B)->default_value(0), "color to draw ref_scan")
                ("general.min_scan_readings", po::value(&min_scan_readings)->default_value(8),
                 "minimum number of readings that should be in the scans before matching to proceed with matching test")
                ("general.resample_scans", po::value<bool>(&resample_scans)->default_value(true),
                 "set true to enable resampling of scans ")
                ("general.resample_distance", po::value(&resample_distance)->default_value(0.03), "resample distance")
                ("general.median_filter_scans", po::value<bool>(&median_filter_scans)->default_value(true),
                 "set true to enable filtering of scans with median filter")
                ("general.median_filter_distance", po::value(&median_filter_distance)->default_value(0.06),
                 "median filter distance")
                ("general.median_filter_order", po::value(&median_filter_order)->default_value(3),
                 "median filter order")
                ("general.ref_scan_odd_readings_only",
                 po::value<bool>(&ref_scan_odd_readings_only)->default_value(false),
                 "set true to take only every second reading starting from reading 1, thus reducing ref_scan number of readings by half")
                ("general.ref_scan_even_readings_only",
                 po::value<bool>(&ref_scan_even_readings_only)->default_value(false),
                 "set true to take only every second reading starting from reading 1, thus reducing ref_scan number of readings by half")
                ("general.new_scan_odd_readings_only",
                 po::value<bool>(&new_scan_odd_readings_only)->default_value(false),
                 "set true to take only every second reading starting from reading 1, thus reducing new_scan number of readings by half")
                ("general.new_scan_even_readings_only",
                 po::value<bool>(&new_scan_even_readings_only)->default_value(false),
                 "set true to take only every second reading starting from reading 1, thus reducing new_scan number of readings by half")
                ("general.fakeError", po::value<std::string>(&fakeError)->default_value("random"),
                 "type of fake Error to introduce in new scan position")
                ("general.maxFakeErrorX", po::value(&maxFakeErrorX)->default_value(0.2),
                 "maximum added error (initial perturbation) in X direction (meters)")
                ("general.maxFakeErrorY", po::value(&maxFakeErrorY)->default_value(0.2),
                 "maximum added error (initial perturbation) in X direction (meters)")
                ("general.maxFakeErrorRot", po::value(&maxFakeErrorRot)->default_value(0.2),
                 "maximum added error (initial perturbation) in Rot (radians)")
                ("general.success_treshold_traslation", po::value(&success_treshold_traslation)->default_value(0.1),
                 "A matching is considered a failure when the solution is larger than this value in translation (meters)")
                ("general.success_treshold_rotation", po::value(&success_treshold_rotation)->default_value(0.01),
                 "A matching is considered a failure when the solution is larger than this value in rotation (rad)");
        ...
        while (i < MAXALGS) {                          //  to add options for a group of algorithms in ini file
            ...
            strcpy(&base_algorithm[strlen(base_algorithm)],
                   ".base_algorithm"); // base_algorithm="algorithm_i.base_algorithm"
            strcpy(parameters_file, base);
            strcpy(&parameters_file[strlen(parameters_file)],
                   ".parameters_file"); // parameters_file="algorithm_i.parameters_file"

            my_options.add_options()
                    (name, po::value<std::string>(&(a[i].name))->default_value("-1"), "algorithm name")
                    (base_algorithm, po::value<std::string>(&(a[i].base_algorithm))->default_value("-1"),
                     "algorithm from which this one is derived")
                    (parameters_file, po::value<std::string>(&(a[i].parameters_file))->default_value("-1"),
                     "parameters_file from which to load algorithm parameters");
    ...
    std::cout << "Read general options" << std::endl;
    ...
    while (i < MAXALGS) {    /*************************************
		...
		*************************************/
        ...
        } else if (a[i].base_algorithm.compare("glasm") == 0 || a[i].base_algorithm.compare("\"glasm\"") == 0) {
        ...
        } else if (a[i].base_algorithm.compare("icp") == 0 || a[i].base_algorithm.compare("\"icp\"") == 0) {
        ...
        } else if (a[i].base_algorithm.compare("mbicp") == 0 || a[i].base_algorithm.compare("\"mbicp\"") == 0) {
        ...
        } else if (a[i].base_algorithm.compare("hglasm") == 0 || a[i].base_algorithm.compare("\"hglasm\"") == 0) {
        ...
        } else if (a[i].base_algorithm.compare("pgambicp") == 0 || a[i].base_algorithm.compare("\"pgambicp\"") == 0) {
        ...
        } else if (a[i].base_algorithm.compare("polarga") == 0 || a[i].base_algorithm.compare("\"polarga\"") == 0) {
    ...
    std::cout << "Finished parameter read" << std::endl;
}
...
*/
    ...
    if (scan_format.compare("sm_cli") == 0) {
        ...
        if (verbose_level > 0) {
            std::cout << "[load_scan_pair            ] Number of scans found in " << scan_path << " folder: " << N
                      << std::endl;
        ...
        if (sr.compare("random") == 0) {
            ...
            if (verbose_level > 0)
                std::cout << "in randomrepeat8 pair%8==0 " << pair % 8 << " pair " << pair << std::endl;
            if (pair % 8 == 0) {
                r = rand() % N;
                if (r + new_scan_offset >= N) r -= new_scan_offset;  // to be sure that new scan will be in too
            }
        ...
        // new_scan_filename
        if (sn.compare("offset") == 0) {   // new_scan is at some offset from ref_scan
            // find ref_scan index
            std::string s = sr;
        ...
        if (!file_loaded) {
            if (verbose_level > 0)
                std::cout << "Loading from rawseeds format file: " << scan_path + scan_basename << std::endl;
    ...
    if (ref_scan_odd_readings_only) scanDecimate(ts1, 2, 1);
    else if (ref_scan_even_readings_only)
        scanDecimate(ts1, 2, 0);

    if (new_scan_odd_readings_only) scanDecimate(ts2, 2, 1);
    else if (new_scan_even_readings_only)
        scanDecimate(ts2, 2, 0);
    //std::cout << "read scan ts1 size: " << ts1.readings.size()<<std::endl;
    //std::cout << "read scan ts2 size: " << ts2.readings.size()<<std::endl;
} // load_scan_pair
 ...
 */
    ...
    if (fakeError.compare("random") == 0) {
        ...
        s.pos.y = s.pos.y + ey;
        if (verbose_level > 0)
            std::cout << "added fixed (" << ex << "," << ey << "," << er << ") error to new_scan position";
    ...
    } else if (fakeError.compare("fixed_all_combinations") ==
               0) {    // used to test one scan with all 8 combinations of fixed error (+-ex,+-ey,+-er)
        s.pos.rot = angleUnwrap(s.pos.rot + er);
        s.pos.x = s.pos.x + ex;
        s.pos.y = s.pos.y + ey;
        if (verbose_level > 0)
            std::cout << "added fixed_all_combinations (" << ex << "," << ey << "," << er
                      << ") error to new_scan position";
        ...
        if (verbose_level > 0)
            std::cout << "added fixed (" << maxFakeErrorX << "," << maxFakeErrorY << "," << maxFakeErrorRot
                      << ") error to new_scan position";
    }
    std::cout << std::endl;
...
void add_fake_error_readings(scan &s) {
    ...
    if (fakeError.compare("random") == 0) {
        ...
        readingsChangeRefFrame(s.readings, {0, 0, 0}, {ex, ey, er});
        if (verbose_level > 0)
            std::cout << "added fixed (" << ex << "," << ey << "," << er << ") error to readings of scan";
        ex += estep;    // each call changes error allowing to sweep entire range (for same scan typically)
        if (ex > maxFakeErrorX) {
            ey += estep;
            ex = -maxFakeErrorX;
...
void pre_process_scans() {
    ...
    if (resample_scans)
        if (verbose_level > 0)
            std::cout << "[pre_process_scans         ] filtering out readings out of range" << std::endl;
...
//// end temp
    ...
    for (unsigned pair = loop_start; NumValidPairs <= loop_end; pair++) {   /* main testing loop:
        ...
        if (ws1.readings.size() < min_scan_readings || ws2.readings.size() < min_scan_readings) {
            if (verbose_level > 0)
                std::cout << "[main loop                 ] this scan pair has too few readings, skipping ..."
                          << std::endl;
            continue;
        }
        ...
        for (unsigned alg = 0;
             alg < NumAlgorithms; alg++) {    // restore working scans - all algorithms use the same scans
            ws1.copy(sws1);
            ws2.copy(sws2);
            ...
            if (verbose_level > 0) {
                ...
                std::cout << "\tEstimated position: \t\t\t(" << mres.p.x << "," << mres.p.y << "," << mres.p.rot << ")"
                          << std::endl;
            ...
            if (!isnan(mres.p.x) && !isnan(mres.p.y) && !isnan(mres.p.rot)) {
                ...
                if (draw_scans) {
                    ...
                    drawing img(filename, map_size_x, map_size_y, -map_size_x / 2.0, -map_size_y / 2.0,
                                map_filename.c_str());
                    img.grid();
                    img.setpointsize(1);
                ...
                if (verbose_level > 0)
                    std::cout << "\tDistance from true pos (dx,dy,drot): \t(" << mres.p.x - ex << "," << mres.p.y - ey
                              << "," << rot[alg][NumValidPairs] << ")  dist:" << dist[alg][NumValidPairs] << std::endl;
            } else {
    ...
    for (unsigned alg = 0; alg < NumAlgorithms; alg++) {
        ...
        for (unsigned pair = 0; pair <
                                NumValidPairs; pair++) {   //std::cout<<"dist["<<alg<<"]["<<pair<<"]: "<<dist[alg][pair]<<"  rot["<<alg<<"]["<<pair<<"]: "<<rot[alg][pair];
        ...
        if (verbose_level > 0) {
            std::cout << "successes(" << algorithm[alg].successes << ")  failures(" << algorithm[alg].failures << ")"
                      << std::endl;
            std::cout << "false_positives(" << algorithm[alg].false_positives << ")  true_positives("
                      << algorithm[alg].true_positives << ")" << std::endl;
            std::cout << "false_negatives(" << algorithm[alg].false_negatives << ")  true_negatives("
                      << algorithm[alg].true_negatives << ")" << std::endl;
        }
#ifdef ERRvsITER
        ...
        SuccessRatio[alg] = algorithm[alg].successes /
                            double(NumValidPairs); // NumValidPairs is guaranteed to be > 0, otherwise we wouldn't be in here

        // variances
        for (unsigned pair = 0; pair < NumValidPairs; pair++) {
            if (is_failure(dist[alg][pair], rot[alg][pair],
                           iter[alg][pair])) {   // skip distance and rotation variance calculation (consider variances only for successful matchings)
                algorithm[alg].vtime_failure += (avg_time[alg][pair] - algorithm[alg].mtime_failure) *
                                                (avg_time[alg][pair] - algorithm[alg].mtime_failure);
                algorithm[alg].viter_failure += (iter[alg][pair] - algorithm[alg].miter_failure) *
                                                (iter[alg][pair] - algorithm[alg].miter_failure);
            } else {
                algorithm[alg].vdist +=
                        (dist[alg][pair] - algorithm[alg].mdist) * (dist[alg][pair] - algorithm[alg].mdist);
                algorithm[alg].vrot += (rot[alg][pair] - algorithm[alg].mrot) * (rot[alg][pair] - algorithm[alg].mrot);
                algorithm[alg].vtime_success += (avg_time[alg][pair] - algorithm[alg].mtime_success) *
                                                (avg_time[alg][pair] - algorithm[alg].mtime_success);
                algorithm[alg].viter_success += (iter[alg][pair] - algorithm[alg].miter_success) *
                                                (iter[alg][pair] - algorithm[alg].miter_success);
#ifdef ERRvsITER
